...
Endpoints under /api/v6/moon-phase/*.
"""
...
@router.post("/api/v6/moon-phase", response_model=MoonPhaseResponseModel)
async def moon_phase(request_body: MoonPhaseRequestModel, request: Request) -> JSONResponse:
    """
    ...
    **Returns:**
    ...
    """
    log_request_with_body(logger, request, "Moon phase request", request_body.model_dump_json())

    try:
        overview = await run_heavy(create_moon_phase_overview, request_body)
...
@router.post("/api/v6/moon-phase/context", response_model=MoonPhaseContextResponseModel)
async def moon_phase_context(request_body: MoonPhaseRequestModel, request: Request) -> JSONResponse:
    """
    **POST** `/api/v6/moon-phase/context`
    ...
    **Returns:**
    ...
    - `moon_phase_overview`: MoonPhaseOverviewModel
    """
    log_request_with_body(logger, request, "Moon phase context request", request_body.model_dump_json())

    try:
        overview = await run_heavy(create_moon_phase_overview, request_body)
        return JSONResponse(content=moon_phase_context_payload(overview), status_code=200)
    except Exception as exc:  # pragma: no cover - defensive
        return await handle_exception(exc, request)


@router.post("/api/v6/moon-phase/now-utc", response_model=MoonPhaseResponseModel)
async def moon_phase_now_utc(request_body: NowMoonPhaseRequestModel, request: Request) -> JSONResponse:
    """
    **POST** `/api/v6/moon-phase/now-utc`
    ...
    **Returns:**
    ...
    - `moon_phase_overview`: MoonPhaseOverviewModel
    """
    log_request_with_body(logger, request, "Moon phase now-utc request", request_body.model_dump_json())
...
@router.post("/api/v6/moon-phase/now-utc/context", response_model=MoonPhaseContextResponseModel)
async def moon_phase_now_utc_context(request_body: NowMoonPhaseRequestModel, request: Request) -> JSONResponse:
    """
    **POST** `/api/v6/moon-phase/now-utc/context`
